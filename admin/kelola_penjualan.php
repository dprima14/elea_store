...
// Hitung per status untuk badge jumlah
$count_status = [];
    ...
    <div style="display:flex;gap:.5rem;flex:1;flex-wrap:wrap;">
        <a href="kelola_penjualan.php?sub=data"
           style="padding:.375rem 1rem;border-radius:.625rem;font-size:.8125rem;font-weight:600;text-decoration:none;
                  <?= $sub==='data' ? 'background:linear-gradient(135deg,#7a2e22,#9e5848);color:white;' : 'background:white;color:#6b7280;border:1px solid #e5e7eb;' ?>">
            <i class="fas fa-clipboard-list"></i> Data Penjualan
        </a>
        <a href="kelola_penjualan.php?sub=rekap"
           style="padding:.375rem 1rem;border-radius:.625rem;font-size:.8125rem;font-weight:600;text-decoration:none;
                  <?= $sub==='rekap' ? 'background:linear-gradient(135deg,#7a2e22,#9e5848);color:white;' : 'background:white;color:#6b7280;border:1px solid #e5e7eb;' ?>">
...
<div id="exportCard" class="cc" style="display:none;border-color:#f2e8dc;margin-bottom:1rem;">
    <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:.75rem;">
        <div>
            ...
            <p style="font-size:.875rem;color:#6b7280;margin:.25rem 0 0;">File berisi 4 sheet: Ringkasan, Detail Transaksi, Per Produk, Per Bulan.</p>
        </div>
    ...
    <form action="export_excel.php" method="GET" style="display:flex;gap:.75rem;align-items:flex-end;flex-wrap:wrap;">
        <div>
            ...
            <input type="date" name="dari" value="<?= date('Y-01-01') ?>" style="padding:.5rem .75rem;border:1px solid #e5e7eb;border-radius:.625rem;font-size:.9375rem;font-family:inherit;outline:none;background:#f9fafb;">
        </div>
        <div>
            <label style="display:block;font-size:.875rem;font-weight:600;color:#6b7280;margin-bottom:.375rem;">Sampai Tanggal</label>
            <input type="date" name="sampai" value="<?= date('Y-m-d') ?>" style="padding:.5rem .75rem;border:1px solid #e5e7eb;border-radius:.625rem;font-size:.9375rem;font-family:inherit;outline:none;background:#f9fafb;">
        </div>
        <button type="submit" class="btn-sm" style="background:linear-gradient(135deg,#4b5563,#6b7280);color:white;display:flex;align-items:center;gap:.5rem;padding:.5rem 1.25rem;">
...
<div style="display:flex;gap:.375rem;margin-bottom:1rem;flex-wrap:wrap;align-items:center;">
    ...
    <a href="kelola_penjualan.php?sub=data&filter=<?= $k ?>"
       style="padding:.25rem .875rem;border-radius:9999px;font-size:.75rem;font-weight:600;text-decoration:none;display:flex;align-items:center;gap:.375rem;
        ...
        <tr>
            ...
            <td>
                <a href="detail_penjualan.php?id=<?= $p['id_penjualan'] ?>"
                   class="btn-sm" style="background:#fff8f6;color:#7a2e22;border:1px solid #fce9e3;padding:.25rem .625rem;font-size:.7rem;text-decoration:none;border-radius:.5rem;">
...
<div class="cc">
    ...
    <table class="dtbl">
        ...
        <tr>
            ...
            <td><span class="tbadge" style="background:#fff8f6;color:#7a2e22;"><?= $r['jumlah_transaksi'] ?> transaksi</span></td>
