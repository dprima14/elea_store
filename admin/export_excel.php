...
$dari   = (isset($_GET['dari'])   && preg_match('/^\d{4}-\d{2}-\d{2}$/', $_GET['dari']))   ? $_GET['dari']   : date('Y-01-01');
...
// Detail transaksi dengan item yang dibeli
...
function xC($val, $type = 'String', $style = '') {
    ...
    if ($type === 'Number') {
        return "<Cell{$sa}><Data ss:Type=\"Number\">" . (is_numeric($val) ? $val : 0) . "</Data></Cell>";
    }
    return "<Cell{$sa}><Data ss:Type=\"String\">" . xe($val) . "</Data></Cell>";
}

function xBlank($n = 1) {
    return str_repeat('<Cell><Data ss:Type="String"></Data></Cell>', $n);
}
...
// ── HTTP Headers ───────────────────────────────────────────────────────────
$filename = 'Laporan_Penjualan_EleaStore_' . date('Ymd_His') . '.xls';
...
<Workbook xmlns="urn:schemas-microsoft-com:office:spreadsheet"
 ...
 <DocumentProperties xmlns="urn:schemas-microsoft-com:office:office">
  <Title>Laporan Penjualan Elea Store</Title><Author>Elea Store Admin</Author>
 </DocumentProperties>
 <Styles>
  <Style ss:ID="Default"><Font ss:FontName="Calibri" ss:Size="11"/></Style>
  <Style ss:ID="s_blank"><Font ss:FontName="Calibri" ss:Size="11"/></Style>
  ...
  <Style ss:ID="s_section_hdr">
   ...
   <Interior ss:Color="#fce9e3" ss:Pattern="Solid"/>
  </Style>
  <Style ss:ID="s_label">
  ...
  <Style ss:ID="s_td_bold"><Font ss:FontName="Calibri" ss:Size="11" ss:Bold="1"/></Style>
  <Style ss:ID="s_rp"><Font ss:FontName="Calibri" ss:Size="11"/><NumberFormat ss:Format="&quot;Rp &quot;#,##0"/></Style>
  <Style ss:ID="s_rp_bold"><Font ss:FontName="Calibri" ss:Size="11" ss:Bold="1"/><NumberFormat ss:Format="&quot;Rp &quot;#,##0"/></Style>

  <!-- Baris bergantian (zebra) -->
  <Style ss:ID="s_z"><Font ss:FontName="Calibri" ss:Size="11"/><Interior ss:Color="#f9fafb" ss:Pattern="Solid"/><Alignment ss:Vertical="Center" ss:WrapText="1"/></Style>
  <Style ss:ID="s_z_c"><Font ss:FontName="Calibri" ss:Size="11" ss:Color="#6b7280"/><Interior ss:Color="#f9fafb" ss:Pattern="Solid"/><Alignment ss:Horizontal="Center" ss:Vertical="Center"/></Style>
  <Style ss:ID="s_z_rp"><Font ss:FontName="Calibri" ss:Size="11"/><Interior ss:Color="#f9fafb" ss:Pattern="Solid"/><NumberFormat ss:Format="&quot;Rp &quot;#,##0"/></Style>

  <!-- Baris total -->
  <Style ss:ID="s_total_rp">
   <Font ss:FontName="Calibri" ss:Size="12" ss:Bold="1" ss:Color="#4b5563"/>
 ...
 <Worksheet ss:Name="Ringkasan">
  <Table ss:DefaultColumnWidth="160">
   ...
   <Row ss:Height="40">
    <?= xC('ELEA STORE', 'String', 's_store_title') ?>
   </Row>
   <Row>
    <?= xC('Toko Fashion for All', 'String', 's_subtitle') ?>
   </Row>
   <Row><Cell><Data ss:Type="String"></Data></Cell></Row>

   <Row ss:Height="22">
    <Cell ss:StyleID="s_section_hdr" ss:MergeAcross="1"><Data ss:Type="String">  INFORMASI LAPORAN</Data></Cell>
   </Row>
   <Row>
    <?= xC('Periode',        'String', 's_label') ?>
    <?= xC($period_lb,       'String', 's_val')   ?>
   </Row>
   <Row>
    <?= xC('Tanggal Cetak',  'String', 's_label') ?>
    <?= xC(date('d M Y, H:i') . ' WIB', 'String', 's_val') ?>
   </Row>
   <Row>
    <?= xC('Dibuat Oleh',    'String', 's_label') ?>
    <?= xC('Admin · Elea Store', 'String', 's_val') ?>
   </Row>
   <Row><Cell><Data ss:Type="String"></Data></Cell></Row>

   <Row ss:Height="22">
    <Cell ss:StyleID="s_section_hdr" ss:MergeAcross="1"><Data ss:Type="String">  RINGKASAN KINERJA</Data></Cell>
   </Row>
   <Row ss:Height="44">
    <?= xC('Total Transaksi',         'String', 's_label')    ?>
    <?= xC((int)$ring['jml'],         'Number', 's_stat_num') ?>
   </Row>
   <Row ss:Height="44">
    <?= xC('Total Pendapatan',        'String', 's_label')    ?>
    <?= xC((float)$ring['total'],     'Number', 's_stat_rp')  ?>
   </Row>
   <Row ss:Height="36">
    <?= xC('Rata-rata per Transaksi', 'String', 's_label')       ?>
    <?= xC((float)$avg_trx,           'Number', 's_stat_rp_sm')  ?>
   </Row>
   <Row ss:Height="36">
    <?= xC('Produk Berbeda Terjual',  'String', 's_label')    ?>
    <?= xC(count($per_produk),        'Number', 's_stat_num') ?>
   </Row>
  </Table>
  <WorksheetOptions xmlns="urn:schemas-microsoft-com:office:excel">
   <FreezePanes/><FrozenNoSplit/><SplitHorizontal>0</SplitHorizontal>
  </WorksheetOptions>
 </Worksheet>
 ...
 <Worksheet ss:Name="Detail Transaksi">
  <Table>
   ...
   <Column ss:Width="130"/>
   <Row ss:Height="26">
    <?= xC('No.',              'String', 's_th_green') ?>
    <?= xC('ID Transaksi',     'String', 's_th_green') ?>
    <?= xC('Tanggal',          'String', 's_th_green') ?>
    <?= xC('Waktu',            'String', 's_th_green') ?>
    <?= xC('Nama Pelanggan',   'String', 's_th_green') ?>
    <?= xC('Produk Dibeli',    'String', 's_th_green') ?>
    <?= xC('Total Harga (Rp)', 'String', 's_th_green') ?>
   </Row>
 ...
 <Worksheet ss:Name="Per Produk">
  <Table>
   ...
   <Row ss:Height="26">
    ...
    <?= xC('Nama Produk',           'String', 's_th_blue') ?>
    <?= xC('Total Terjual (unit)',   'String', 's_th_blue') ?>
    <?= xC('Total Pendapatan (Rp)', 'String', 's_th_blue') ?>
   </Row>
   ...
   <?php foreach ($per_produk as $i => $p):
     ...
     $tc = $ev ? 's_td_c' : 's_z_c';
     $rp = $ev ? 's_rp'   : 's_z_rp';
     $tot_unit += (int)$p['total_terjual'];
     $tot_pend += (float)$p['total_pendapatan'];
   ?>
   <Row>
    <?= xC($i + 1, 'Number', $tc) ?>
 ...
 <Worksheet ss:Name="Per Bulan">
  <Table>
   ...
   <Column ss:Width="170"/>
   <Row ss:Height="26">
    <?= xC('No.',                      'String', 's_th_purple') ?>
    <?= xC('Bulan',                    'String', 's_th_purple') ?>
    <?= xC('Jumlah Transaksi',         'String', 's_th_purple') ?>
    <?= xC('Total Pendapatan (Rp)',    'String', 's_th_purple') ?>
    <?= xC('Rata-rata/Transaksi (Rp)', 'String', 's_th_purple') ?>
   </Row>
   ...
   <?php foreach ($per_bulan as $i => $b):
     ...
     $tot_trx    += (int)$b['jml_trx'];
     $tot_pend_b += (float)$b['total'];
   ?>
   <Row>
    <?= xC($i + 1, 'Number', $tc) ?>
