...
<style>
.dp-wrap{display:grid;grid-template-columns:1fr 320px;gap:1.25rem;align-items:start;}
...
.tl-dot.done{background:#7a2e22;border-color:#7a2e22;color:white;}
.tl-dot.active{background:white;border-color:#7a2e22;color:#7a2e22;}
...
.status-form{display:flex;gap:.5rem;flex-wrap:wrap;margin-top:1rem;}
.status-btn{padding:.4rem 1rem;border-radius:.5rem;border:1.5px solid;font-size:.75rem;font-weight:600;cursor:pointer;font-family:inherit;transition:all .15s;}
.map-mini{height:180px;border-radius:.625rem;overflow:hidden;border:1px solid #e5e7eb;margin-top:.75rem;}
...
<div style="margin-bottom:1rem;display:flex;align-items:center;gap:1rem;flex-wrap:wrap;">
    <a href="kelola_penjualan.php" style="font-size:.8125rem;color:#7a2e22;font-weight:600;text-decoration:none;">← Kembali ke Data Penjualan</a>
...
<div>
    ...
    <div class="dp-card">
        <h3><i class="fas fa-map-marker-alt" style="color:#7a2e22;"></i> Informasi Pengiriman</h3>
        <div class="dp-grid">
            <div class="dp-item">
            ...
            <div class="dp-item full">
                ...
                <span class="dp-val" style="font-size:.75rem;color:#6b7280;">
                    ...
                    <a href="https://www.openstreetmap.org/?mlat=<?= $penjualan['lat'] ?>&mlon=<?= $penjualan['lng'] ?>#map=16/<?= $penjualan['lat'] ?>/<?= $penjualan['lng'] ?>"
                       target="_blank" style="color:#7a2e22;font-weight:600;text-decoration:none;margin-left:.5rem;">
    ...
    <!-- PRODUK -->
    <div class="dp-card">
        <h3><i class="fas fa-shopping-bag" style="color:#7a2e22;"></i> Item Produk</h3>
        ...
        <div class="prod-row">
            ...
            <div style="font-weight:700;color:#7a2e22;font-size:.875rem;"><?= fmt_rp((int)$d['subtotal']) ?></div>
        </div>
        <?php endforeach; ?>
        <div style="display:flex;justify-content:space-between;padding:.75rem 0 0;border-top:2px solid #f3f4f6;margin-top:.5rem;">
            ...
            <span style="font-weight:700;color:#7a2e22;font-size:1.0625rem;"><?= fmt_rp((int)$penjualan['total_harga']) ?></span>
        </div>
    </div>
...
<div>
    ...
    <!-- STATUS & UBAH STATUS -->
    <div class="dp-card">
        <h3><i class="fas fa-truck" style="color:#7a2e22;"></i> Status Pengiriman</h3>
        ...
        <div class="timeline">
            ...
            <div class="tl-item">
                <div class="tl-dot <?= $cls ?>">
                    <i class="fas fa-<?= $done && !$active ? 'check' : ($active ? 'circle' : 'circle') ?>"></i>
                </div>
                <div class="tl-content">
                    <div class="tl-label" style="<?= $active ? 'color:#7a2e22;font-weight:700;' : ($done ? '' : 'color:#9ca3af;') ?>"><?= $label ?></div>
        ...
        <div style="margin-top:1.25rem;padding-top:1rem;border-top:1px solid #f3f4f6;">
            ...
            <form method="POST">
                ...
                <div style="display:flex;flex-direction:column;gap:.375rem;">
                    <?php
                    $btn_styles = [
                        'pending'    => 'border-color:#fde68a;color:#d97706;background:white;',
                        'diproses'   => 'border-color:#bfdbfe;color:#2563eb;background:white;',
                        'dikirim'    => 'border-color:#bae6fd;color:#0369a1;background:white;',
                        'selesai'    => 'border-color:#86efac;color:#15803d;background:white;',
                        'dibatalkan' => 'border-color:#fca5a5;color:#dc2626;background:white;',
                    ];
    ...
    <!-- INFO PEMBAYARAN -->
    <div class="dp-card">
        <h3><i class="fas fa-credit-card" style="color:#7a2e22;"></i> Pembayaran</h3>
        ...
        <div class="dp-item">
            ...
            <span style="font-size:1.125rem;font-weight:700;color:#7a2e22;"><?= fmt_rp((int)$penjualan['total_harga']) ?></span>
        </div>
    </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    map.setView(ll, 16);
    L.marker(ll).addTo(map).bindPopup('<?= htmlspecialchars(addslashes($penjualan['alamat_pengiriman'] ?? 'Titik pengiriman')) ?>').openPopup();
});
</script>
<?php endif; ?>
